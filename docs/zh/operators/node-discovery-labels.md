...
省略该字段时，规范场景字节与摘要保持不变；既有场景修订版不受影响。但该节点组下每个模拟节点**仍会出现已声明的厂商键，只是值为空**——键集合由契约固定，只有取值是可选投射的：这个开关决定的是「是否投射取值」，而不是「键是否存在」。
...
厂商标签的**键**在契约层声明，**取值**在型号层提供。已声明但该型号没有证据的键仍会被投射，只是值为空——例如 MI300X 的 `amd.com/gpu.family`、Atlas A2 的 `accelerator`，以及 `hygon.com/dcu*` 的全部键。节点组从未开启 `discoveryLabels` 的节点会到达同一种可观测状态：所有已声明键都存在且为空。两种情况都是有意为之：
