...
即使打开浏览器失败，终端打印的 URL 仍可使用。监听地址默认是
`127.0.0.1`。只有确实需要从其他机器访问时，才通过 `--host` 指定一个不含端口
的 IP 地址或主机名：
...
非回环监听使用未加密 HTTP，并会输出安全警告。请限制网络访问并保护完整能力
URL。使用通配地址时，应把输出 URL 中的 `0.0.0.0` 替换为一个可达 IP；通配
监听会主动拒绝 DNS Host。
...
标题栏主题按钮可在浅色与深色配色之间切换。URL 没有 `theme` 参数时，页面跟随
操作系统主题，并随系统偏好变化实时更新；用户显式选择时只把 `theme=light` 或
`theme=dark` 写入查询字符串，因此刷新和浏览器前进/后退可以恢复主题，同时不
存储能力 token 或任何集群数据。两个主题在桌面和移动端保持相同的证据层级、
可见焦点、状态文本和至少 44px 的主要操作目标。
...
Bearer 凭据发送。服务端只接受 `GET` 和 `HEAD`，校验选定的 Host 和端口，不
提供写接口或宽松 CORS，禁止缓存，并且只提供内嵌的 HTML、CSS 和 JavaScript。
通配监听只接受 IP 字面量 Host。按 `Ctrl+C` 停止；关闭过程会取消 watch，并在
五秒内结束。

终端打印的 URL 应按临时密钥处理。在进程结束前，任何能访问监听地址并使用该
URL 的人都可以读取同样的集群元数据。
...
桌面与 390 px 布局、中英文、系统与显式浅色/深色主题、URL 历史、键盘与焦点、
部分数据、同源请求、fragment 令牌隔离、无 JavaScript fallback，以及 100 行
DOM 上限。CI 会保留四张截图作为视觉证据：
