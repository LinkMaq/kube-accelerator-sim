...
`kasim ui` is a temporary, read-only view of one explicit Simulation Target, not a persistent management service. The released `kasim` binary will embed standards-based HTML, CSS, and JavaScript assets and serve them only from `127.0.0.1`. It will not require a Node.js runtime, an external asset directory, another process, a Helm component, remote authentication, or a non-loopback listen option.
