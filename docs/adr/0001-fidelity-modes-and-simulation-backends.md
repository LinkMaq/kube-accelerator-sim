...
KWOK is the strongest current substrate for high-scale Synthetic Node lifecycle, but it is an implementation dependency rather than a user intent. A minimal native reconciler remains useful as a prototype challenger, not yet as a second supported backend.
...
A project-pinned KWOK release is the private default Synthetic Node implementation. KWOK annotations, Stages, object choreography, and version details do not enter the Scenario, Vendor Profile, CLI, or public Module interface. The native reconciler is evaluated by prototype; a `SyntheticRuntime` Seam is introduced only if that prototype becomes a second maintained implementation. kind is an End-to-End Test Harness, not a product backend.
