...
Types, native label sets, units, and normally HELP strings come from a
source-backed Telemetry Contract while every value is generated by Kasim;
documented compatibility value conventions may carry compatibility HELP.
Per-device
families add only the cross-exporter `node` compatibility label, whose value is
the Synthetic Node; separate `kasim_telemetry_*` diagnostics expose simulation
provenance. It is explicitly not a physical sensor reading, vendor
exporter process, driver observation, performance result, or accelerator-compute
claim.
...
Prometheus metric families, HELP strings, types, units, common and
family-specific native labels, product gates, and
