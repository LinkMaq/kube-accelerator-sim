...
func TestTelemetryDeliveryModesFailClosed(t *testing.T) {
	...
	serviceMonitor := requireObject(
		t,
		withCRD,
		"ServiceMonitor",
		"contract-kasim-runtime-telemetry",
	)
	endpoints := sliceValue(t, serviceMonitor.Spec, "endpoints")
	if len(endpoints) != 1 {
		t.Fatalf("ServiceMonitor endpoints = %d, want 1", len(endpoints))
	}
	endpoint, ok := endpoints[0].(map[string]any)
	if !ok {
		t.Fatalf("ServiceMonitor endpoint has type %T, want map", endpoints[0])
	}
	relabelings := sliceValue(t, endpoint, "metricRelabelings")
...
func TestChartVersionsAndImmutableRuntimeInputsStayExplicit(t *testing.T) {
	...
	for key, want := range map[string]string{
		...
		"simulation.kasim.io/catalog-revision":           "2026-09-05.1",
		"simulation.kasim.io/telemetry-catalog-revision": "2026-09-05.1",
		"simulation.kasim.io/telemetry-catalog-sha256":   "87ef01b69d3d1e6c742f587be4d6c60be1080b548b48aa32ed896ffd1eb37abd",
